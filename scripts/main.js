$(function() {
  ...
  $('#share-close-btn, #share-backdrop, #share-cancel-btn').on('click', () => UI.closeShareModal());
  ...
  $('#chat-toggle-btn, #chat-close-btn, #chat-backdrop, #open-chef-btn').on('click', () => UI.toggleChat());
    ...
    try {
      ...
      await LLM.generate(messages, {
        ...
        onToken: (token) => {
          ...
          const displayHtml = aiResponse.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>').replace(/\n/g, '<br>');
      ...
      $contentDiv.html('<span class="text-red-500">Oops, I got distracted. Please try again.</span>');
      console.error(err);
    }
  });
  ...
  $('#analyze-btn').on('click', function() {
    ...
    const prompt = "Based on my current shopping list, what can I cook? Give me one creative recipe idea.";
